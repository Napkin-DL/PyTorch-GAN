...
# Optimizers
target_encode_optimizer_G = torch.optim.Adam(target_encode_generator.parameters(),lr=opt.lr, betas=(opt.b1, opt.b2))
source_encode_optimizer_G = torch.optim.Adam(source_encode_generator.parameters(),lr=opt.lr, betas=(opt.b1, opt.b2))
target_decode_optimizer_G = torch.optim.Adam( itertools.chain(target_decode_generator.parameters(), classifier.parameters()),
                                lr=opt.lr, betas=(opt.b1, opt.b2))
source_decode_optimizer_G = torch.optim.Adam( itertools.chain(source_decode_generator.parameters(), classifier.parameters()),
                                lr=opt.lr, betas=(opt.b1, opt.b2))
optimizer_D = torch.optim.Adam(encode_discriminator.parameters(), lr=opt.lr, betas=(opt.b1, opt.b2))
optimizer_D = torch.optim.Adam(encode_discriminator.parameters(), lr=opt.lr, betas=(opt.b1, opt.b2))
...
for epoch in range(opt.n_epochs):
    for i, ((imgs_A, labels_A), (imgs_B, labels_B)) in enumerate(zip(dataloader_A, dataloader_B)):
        ...
        target_encode_optimizer_G.zero_grad()
