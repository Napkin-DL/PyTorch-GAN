...
class sdecode_ResidualBlock(nn.Module):
    def __init__(self, x, in_features=64, out_features=64):
...
class target_decode_Generator(nn.Module):
    ...
    def forward(self, encode_out):
        out = self.target_decode_resblocks(encode_out)
        img_ = self.tl2(out)
...
class source_decode_Generator(nn.Module):
    ...
    def forward(self, img, encode_out):
        out = self.source_decode_resblocks(encode_out)
        img_ = self.sl2(out)
...
for epoch in range(opt.n_epochs):
    for i, ((imgs_A, labels_A), (imgs_B, labels_B)) in enumerate(zip(dataloader_A, dataloader_B)):
        ...
        decode_fake_B = source_decode_generator(encode_fake_B)
        ...
        decode_real_B = target_decode_generator(encode_real_B)
