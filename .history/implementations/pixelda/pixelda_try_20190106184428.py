...
class encode_Generator(nn.Module):
    ...
    def forward(self, img, z):
        ...
        x, encode_x = self.l1(gen_input)
        encode_out1 = self.encode_resblocks1(encode_x)
        encode_out2 = self.encode_resblocks2(encode_out1)
        encode_out = self.encode_resblocks3(encode_out2)

        return img, encode_out
...
class decode_Generator(nn.Module):
    ...
    def forward(self, img, z):
        ...
        encode_out1 = self.encode_resblocks1(encode_x)
        encode_out2 = self.encode_resblocks1(encode_out1)
        encode_out3 = self.encode_resblocks1(encode_out2)
        decode_out1 = encode_out3 + self.decode_resblocks1(encode_out3)
        decode_out2 = encode_out2 + self.decode_resblocks2(decode_out1)
        decode_out3 = encode_out1 + self.decode_resblocks3(decode_out2)
