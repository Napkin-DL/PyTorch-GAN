...
class decode_ResidualBlock2(nn.Module):
    def __init__(self, in_features=256, out_features=64, kernel_size=3, stride=2, padding=0):
...
class decode_ResidualBlock3(nn.Module):
    def __init__(self, in_features=128, out_features=32, kernel_size=3, stride=2, padding=0):
...
class encode_Discriminator(nn.Module):
    def __init__(self):
        ...
        self.model = nn.Sequential(
            *block(256, 64, normalization=False),
            *block(64, 128),
            *block(128, 256),
            *block(256, 512),
...
source_recon_loss = torch.nn.MSELoss()
target_recon_loss = torch.nn.MSELoss()
...
for epoch in range(opt.n_epochs):
    for i, ((imgs_A, labels_A), (imgs_B, labels_B)) in enumerate(zip(dataloader_A, dataloader_B)):
        ...
        target_recons = target_recon_loss(decode_real_B, imgs_B)
